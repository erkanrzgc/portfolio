...
export default function NetworkGlobe({ className = '' }: NetworkGlobeProps) {
  ...
  useEffect(() => {
    ...
    const resize = () => {
      ...
      const nextWidth = Math.max(1, Math.round(width || container.clientWidth))
      const nextHeight = Math.max(1, Math.round(height || container.clientHeight))

      camera.aspect = nextWidth / nextHeight
      camera.updateProjectionMatrix()
      renderer.setSize(nextWidth, nextHeight, false)

      if (prefersReducedMotion || !animationFrame) renderer.render(scene, camera)
    }
